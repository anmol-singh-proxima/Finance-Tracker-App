...
| **BR-16** | Calendar-based expense management | A user manages expenses through a **month-view calendar** (like Google/Apple Calendar): pick any month/year, see each day's expense count and total at a glance, and open any day to **view** its expenses in a table or **edit** them in place (modify, add, delete — with confirmation before deleting). Changes appear immediately without a page refresh. **Only today and past dates are interactive** — future dates appear visually disabled and cannot be viewed or edited. | S |
| **BR-17** | Global currency preference | A user can pick their preferred **display currency** from a selector in the application header; every monetary value in the app (dashboard, expenses, calendar, dialogs, investments, charts) renders in that currency, and the choice **persists across sessions**. This is a display/format preference only — stored amounts are not converted between currencies. Adding a new supported currency must require only a configuration change. | S |
| **BR-18** | Category management | A user manages categories in a dedicated **Categories** module: list all categories in a table and view, create, edit, and delete them. Each category has a **type** (Expense or Investment) and may optionally have **one parent category** (single-level hierarchy, e.g. Utilities → Electricity Bill). Expense screens offer only Expense categories; investment screens only Investment categories, with parents and subcategories both selectable and visually distinguished. A category that is **in use** by any expense/investment record (or has subcategories) **cannot be deleted**, with a clear explanation; unused categories delete only after confirmation. Predefined categories cannot be edited or deleted. | S |
