...
| **IMPL-FE-09** | `src/components/Calendar/`, `src/components/Dialogs/`, `src/utils/calendar.ts` | Month-view expense calendar: grid + day cells, day View/Edit dialogs, delete confirmation, accessible modal base. Future dates render disabled (no view/edit). | BR-16, BR-02/05/11 | Calendar math is pure/unit-tested; dialogs stage edits and apply them only on Save. Follows UI-UX-DESIGN-STANDARDS.md. |
| **IMPL-FE-10** | `src/pages/Categories.tsx` | Category management module: typed table (parents + indented subcategories), view/create/edit dialogs, safe deletion | BR-18, BR-03 | Predefined rows are read-only; deletion pre-checked client-side (linked count/children), enforced server-side (IMPL-BE-11). |
| **IMPL-FE-11** | `src/utils/currencies.ts`, `src/store/slices/settingsSlice.ts`, `src/hooks/useCurrencyFormatter.ts` | Global display-currency preference: supported-currency config, persisted setting, formatter hook used by every money render | BR-17 | Display/format only — no FX conversion. Persisted in localStorage; new currency = one config entry. |
| **IMPL-FE-12** | `src/index.css` (design tokens), `src/charts/palette.ts`, `src/components/Common/Navigation.tsx` (+ user icon, currency selector), `src/components/Common/Footer.tsx`, shared `.page` container | Centralized theme + app shell: tokenized colors/radii/shadows, sticky header & footer, uniform page layout | TR-UX-01/02, BR-15 | No hardcoded color literals outside the token definitions; Light/Dark becomes a token-level change. |
...
| **IMPL-BE-05** | `routers/investments.py`, `services/investment_service.py`, `repositories/investment_repo.py` | Investment CRUD + summary, scoped to the user id | BR-04/06/11, BR-18, TR-DAT-01 | `type` must name an investment category (validated at write time, BR-18). |
...
| **IMPL-BE-11** | `routers/categories.py`, `services/category_service.py`, `repositories/category_repo.py` | Category management: typed (expense/investment) CRUD, single-level hierarchy (`parent_id`), per-user linked-record counts, safe deletion (in-use/parent/predefined → 409); (later) budgets & alerts | BR-03, BR-18, BR-12 | Renames propagate to the caller's records (by-name references). Budget/alert logic is BR-12 (Could) — stub now, build later. |
...
|--------|-----------|--------|-------|
| `Expense` | `id`, `user_id`, `category`, `amount`, `date`, `description`, timestamps | BR-02/03 | `category` is a validated string (predefined or user's own **expense** category), not a FK. Index `(user_id, date)`, `(user_id, category)`. |
| `Investment` | `id`, `user_id`, `name`, `type`, `amount`, `current_value`, `purchase_date`, `notes`, timestamps | BR-04, BR-18 | `type` names an **investment** category (validated string, same discipline as expenses; legacy keys normalised by migration `0003`). Index `(user_id, purchase_date)`. |
| `Category` | `id`, `user_id?` (null = predefined), `name`, `type` ('expense'\|'investment'), `parent_id?` (self-FK, RESTRICT), `is_predefined`, `created_at` | BR-03, BR-18 | Predefined (seeded, both types) + per-user custom. Single-level hierarchy; name unique per user **per type** (case-insensitive). Migration `0003`. |
